 ...
 */
JNIEXPORT jbyte JNICALL Java_de_fraunhofer_fokus_ants_jipfix_jIPFIX_ipfixAddVendorInformationElements(JNIEnv * env, jobject self, jobjectArray elements) {
  jsize len = (*env)->GetArrayLength(env,elements);
  ipfix_field_type_t ipfix_fts[len]; 
  int i;
  ...
  jfieldID fname = (*env)->GetFieldID(env, classFT, "name", "java/lang/String");
  jfieldID fdocumentation = (*env)->GetFieldID(env, classFT, "documentation", "java/lang/String");
  ...
  for (i=0;i<len;i++) {
    ...
    jstring sdoc = (*env)->GetObjectField(env,element,fdocumentation);
 ...
 */
 ...
 jobject buildDataRecord(ipfix_datarecord_t *d, ipfixt_node_t *t) {
   ...
   int i=0, nfields = t->ipfixt->nfields;
 ...
 int export_newsource_cb( ipfixs_node_t *s, void *arg ) {
   ...
   (*senv)->ExceptionDescribe(senv); //we've just called a callback! teh user might do anything!
   return 0;
 }
